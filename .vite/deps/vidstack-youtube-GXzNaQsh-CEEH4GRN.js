import __buffer_polyfill from 'vite-plugin-node-polyfills/shims/buffer'
globalThis.Buffer = globalThis.Buffer || __buffer_polyfill
import __global_polyfill from 'vite-plugin-node-polyfills/shims/global'
globalThis.global = globalThis.global || __global_polyfill
import __process_polyfill from 'vite-plugin-node-polyfills/shims/process'
globalThis.process = globalThis.process || __process_polyfill

import {
  W
} from "./chunk-3UNSOOUW.js";
import {
  l
} from "./chunk-LAQXM4Q3.js";
import {
  A3e,
  D1,
  Dt,
  Rt,
  T3e,
  ig,
  kf,
  na,
  uh,
  ye
} from "./chunk-VQLFV2GX.js";
import "./chunk-5BXUT56C.js";
import "./chunk-4OSCJW2H.js";
import "./chunk-HTF5FYCO.js";
import "./chunk-2OJGSHGR.js";
import "./chunk-QI253I2C.js";
import "./chunk-W24ITOCJ.js";
import "./chunk-ESEZ4R5I.js";
import {
  __toESM,
  require_dist,
  require_dist2,
  require_dist3
} from "./chunk-V3F6WX4O.js";

// node_modules/@webitel/ui-sdk/dist/vidstack-youtube-GXzNaQsh.js
var import_dist = __toESM(require_dist(), 1);
var import_dist2 = __toESM(require_dist2(), 1);
var import_dist3 = __toESM(require_dist3(), 1);
var K = Object.defineProperty;
var B = (r) => {
  throw TypeError(r);
};
var N = (r, n, e) => n in r ? K(r, n, { enumerable: true, configurable: true, writable: true, value: e }) : r[n] = e;
var T = (r, n, e) => N(r, typeof n != "symbol" ? n + "" : n, e);
var R = (r, n, e) => n.has(r) || B("Cannot " + e);
var s = (r, n, e) => (R(r, n, "read from private field"), e ? e.call(r) : n.get(r));
var p = (r, n, e) => n.has(r) ? B("Cannot add the same private member more than once") : n instanceof WeakSet ? n.add(r) : n.set(r, e);
var l2 = (r, n, e, t) => (R(r, n, "write to private field"), t ? t.call(r, e) : n.set(r, e), e);
var o = (r, n, e) => (R(r, n, "access private method"), e);
var b = {
  Ended: 0,
  Playing: 1,
  Paused: 2,
  Buffering: 3,
  Cued: 5
};
var a;
var k;
var v;
var S;
var P;
var m;
var y;
var i;
var te;
var se;
var D;
var f;
var j;
var Y;
var F;
var I;
var M;
var x;
var L;
var O;
var V;
var U;
var ne = class extends W {
  constructor(e, t) {
    super(e);
    p(this, i);
    T(this, "$$PROVIDER_TYPE", "YOUTUBE");
    T(this, "scope", uh());
    p(this, a);
    p(this, k, Dt(""));
    p(this, v, -1);
    p(this, S, null);
    p(this, P, -1);
    p(this, m, false);
    p(this, y, /* @__PURE__ */ new Map());
    T(this, "language", "en");
    T(this, "color", "red");
    T(this, "cookies", false);
    l2(this, a, t);
  }
  get currentSrc() {
    return s(this, S);
  }
  get type() {
    return "youtube";
  }
  get videoId() {
    return s(this, k).call(this);
  }
  preconnect() {
    D1(this.getOrigin());
  }
  setup() {
    super.setup(), ye(o(this, i, D).bind(this)), s(this, a).notify("provider-setup", this);
  }
  destroy() {
    o(this, i, O).call(this);
    const e = "provider destroyed";
    for (const t of s(this, y).values())
      for (const { reject: d } of t) d(e);
    s(this, y).clear();
  }
  async play() {
    return o(this, i, f).call(this, "playVideo");
  }
  async pause() {
    return o(this, i, f).call(this, "pauseVideo");
  }
  setMuted(e) {
    e ? o(this, i, f).call(this, "mute") : o(this, i, f).call(this, "unMute");
  }
  setCurrentTime(e) {
    o(this, i, f).call(this, "seekTo", e), s(this, a).notify("seeking", e);
  }
  setVolume(e) {
    o(this, i, f).call(this, "setVolume", e * 100);
  }
  setPlaybackRate(e) {
    o(this, i, f).call(this, "setPlaybackRate", e);
  }
  async loadSource(e) {
    if (!Rt(e.src)) {
      l2(this, S, null), s(this, k).set("");
      return;
    }
    const t = l(e.src);
    s(this, k).set(t ?? ""), l2(this, S, e);
  }
  getOrigin() {
    return this.cookies ? "https://www.youtube.com" : "https://www.youtube-nocookie.com";
  }
  buildParams() {
    const { keyDisabled: e } = s(this, a).$props, { muted: t, playsInline: d, nativeControls: u } = s(this, a).$state, h = u();
    return {
      rel: 0,
      autoplay: 0,
      cc_lang_pref: this.language,
      cc_load_policy: h ? 1 : void 0,
      color: this.color,
      controls: h ? 1 : 0,
      disablekb: !h || e() ? 1 : 0,
      enablejsapi: 1,
      fs: 1,
      hl: this.language,
      iv_load_policy: h ? 1 : 3,
      mute: t() ? 1 : 0,
      playsinline: d() ? 1 : 0
    };
  }
  onLoad() {
    window.setTimeout(() => this.postMessage({ event: "listening" }), 100);
  }
  onMessage({ info: e }, t) {
    var c;
    if (!e) return;
    const { title: d, intrinsicDuration: u, playbackRate: h } = s(this, a).$state;
    if (A3e(e.videoData) && e.videoData.title !== d() && s(this, a).notify("title-change", e.videoData.title, t), na(e.duration) && e.duration !== u()) {
      if (na(e.videoLoadedFraction)) {
        const g = ((c = e.progressState) == null ? void 0 : c.loaded) ?? e.videoLoadedFraction * e.duration, w = new kf(0, e.duration);
        o(this, i, I).call(this, g, w, t);
      }
      s(this, a).notify("duration-change", e.duration, t);
    }
    if (na(e.playbackRate) && e.playbackRate !== h() && s(this, a).notify("rate-change", e.playbackRate, t), e.progressState) {
      const { current: g, seekableStart: w, seekableEnd: E, loaded: J, duration: _ } = e.progressState;
      o(this, i, F).call(this, g, t), o(this, i, I).call(this, J, new kf(w, E), t), _ !== u() && s(this, a).notify("duration-change", _, t);
    }
    if (na(e.volume) && T3e(e.muted) && !s(this, m)) {
      const g = {
        muted: e.muted,
        volume: e.volume / 100
      };
      s(this, a).notify("volume-change", g, t);
    }
    na(e.playerState) && e.playerState !== s(this, v) && o(this, i, L).call(this, e.playerState, t);
  }
};
a = /* @__PURE__ */ new WeakMap(), k = /* @__PURE__ */ new WeakMap(), v = /* @__PURE__ */ new WeakMap(), S = /* @__PURE__ */ new WeakMap(), P = /* @__PURE__ */ new WeakMap(), m = /* @__PURE__ */ new WeakMap(), y = /* @__PURE__ */ new WeakMap(), i = /* @__PURE__ */ new WeakSet(), te = function(e) {
  var t;
  (t = o(this, i, V).call(this, "playVideo")) == null || t.reject(e);
}, se = function(e) {
  var t;
  (t = o(this, i, V).call(this, "pauseVideo")) == null || t.reject(e);
}, D = function() {
  o(this, i, O).call(this);
  const e = s(this, k).call(this);
  if (!e) {
    this.src.set("");
    return;
  }
  this.src.set(`${this.getOrigin()}/embed/${e}`), s(this, a).notify("load-start");
}, f = function(e, t) {
  let d = ig(), u = s(this, y).get(e);
  return u || s(this, y).set(e, u = []), u.push(d), this.postMessage({
    event: "command",
    func: e,
    args: t ? [t] : void 0
  }), d.promise;
}, j = function(e) {
  s(this, a).notify("loaded-metadata"), s(this, a).notify("loaded-data"), s(this, a).delegate.ready(void 0, e);
}, Y = function(e) {
  var t;
  (t = o(this, i, V).call(this, "pauseVideo")) == null || t.resolve(), s(this, a).notify("pause", void 0, e);
}, F = function(e, t) {
  const { duration: d, realCurrentTime: u } = s(this, a).$state, h = s(this, v) === b.Ended, c = h ? d() : e;
  s(this, a).notify("time-change", c, t), !h && Math.abs(c - u()) > 1 && s(this, a).notify("seeking", c, t);
}, I = function(e, t, d) {
  const u = {
    buffered: new kf(0, e),
    seekable: t
  };
  s(this, a).notify("progress", u, d);
  const { seeking: h, realCurrentTime: c } = s(this, a).$state;
  h() && e > c() && o(this, i, M).call(this, d);
}, M = function(e) {
  const { paused: t, realCurrentTime: d } = s(this, a).$state;
  window.clearTimeout(s(this, P)), l2(this, P, window.setTimeout(
    () => {
      s(this, a).notify("seeked", d(), e), l2(this, P, -1);
    },
    t() ? 100 : 0
  ));
}, x = function(e) {
  const { seeking: t } = s(this, a).$state;
  t() && o(this, i, M).call(this, e), s(this, a).notify("pause", void 0, e), s(this, a).notify("end", void 0, e);
}, L = function(e, t) {
  var E;
  const { paused: d, seeking: u } = s(this, a).$state, h = e === b.Playing, c = e === b.Buffering, g = o(this, i, U).call(this, "playVideo"), w = d() && (c || h);
  if (c && s(this, a).notify("waiting", void 0, t), u() && h && o(this, i, M).call(this, t), s(this, m) && h) {
    this.pause(), l2(this, m, false), this.setMuted(s(this, a).$state.muted());
    return;
  }
  if (!g && w) {
    l2(this, m, true), this.setMuted(true);
    return;
  }
  switch (w && ((E = o(this, i, V).call(this, "playVideo")) == null || E.resolve(), s(this, a).notify("play", void 0, t)), e) {
    case b.Cued:
      o(this, i, j).call(this, t);
      break;
    case b.Playing:
      s(this, a).notify("playing", void 0, t);
      break;
    case b.Paused:
      o(this, i, Y).call(this, t);
      break;
    case b.Ended:
      o(this, i, x).call(this, t);
      break;
  }
  l2(this, v, e);
}, O = function() {
  l2(this, v, -1), l2(this, P, -1), l2(this, m, false);
}, V = function(e) {
  var t;
  return (t = s(this, y).get(e)) == null ? void 0 : t.shift();
}, U = function(e) {
  var t;
  return !!((t = s(this, y).get(e)) != null && t.length);
};
export {
  ne as YouTubeProvider
};
//# sourceMappingURL=vidstack-youtube-GXzNaQsh-CEEH4GRN.js.map
