...
describe('"joined" queues api script', () => {
	it('joined: default is today', () => {
		...
		expect(`${joinedAtFrom}`.slice(0, -4)).toEqual(
			`${result.joinedAtFrom}`.slice(0, -4),
		);
		expect(`${joinedAtTo}`.slice(0, -4)).toEqual(
			`${result.joinedAtTo}`.slice(0, -4),
		);
	});
	it('joined: 3hour', () => {
		const end = new Date();
		...
		// checking approx equality cause new Date() time may not be sync
		expect(`${joinedAtFrom}`.slice(0, -3)).toEqual(
			`${result.joinedAtFrom}`.slice(0, -3),
		);
		expect(`${joinedAtTo}`.slice(0, -3)).toEqual(
			`${result.joinedAtTo}`.slice(0, -3),
		);
	});
});
