...
describe('Agent Skills API', () => {
	it('getList: correctly processes response', async () => {
		...
		const response = await AgentSkillsAPI.getList({
			parentId,
		});
	...
	it('add: correctly passes item to axios', async () => {
		instance.request.mockImplementationOnce(() =>
			Promise.resolve({
				data: {
					...itemInstance,
					agentId: parentId,
		...
		const response = await AgentSkillsAPI.add({
			parentId,
			itemInstance,
		});
	...
	it('update: correctly passes item to axios', async () => {
		...
		const response = await AgentSkillsAPI.update({
			parentId,
			itemId: id,
			itemInstance,
		});
	...
	it('patch: correctly passes item to axios', async () => {
		...
		const response = await AgentSkillsAPI.patch({
			parentId,
			id,
			changes: itemInstance,
		});
		expect(response).toStrictEqual(expectItemInstance);
	});
	it('delete: correctly calls axios delete', async () => {
		const deleteMock = instance.request.mockImplementationOnce(() =>
			Promise.resolve({}),
		);
		await AgentSkillsAPI.delete({
			parentId,
			id,
		});
